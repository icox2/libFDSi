...
Implant::Implant() :
        TOF(0), dE(0), time(0), present(false), valid(false), nHits(0), tracelen(0),
        trace(NULL), upperthresh(60000) {
  for (int i = 0; i < 5; ++i) {
    energy[0][i] = 0;
    energy[1][i] = 0;
    energy[2][i] = 0;
...
void Implant::SetMeas(PIXIE::Measurement &meas, int indx) {
  ...
  if (meas.eventEnergy < thresh[indx]) {
    return;
  }
  if (meas.eventEnergy > upperthresh) {
    return;
  }
...
void DSSD::SetMeas(PIXIE::Measurement &meas, int indx) {
  // if(!meas.outOfRange && meas.eventEnergy>60000)
  //   return;
  DSSDhit hit;
  hit.rawenergy = meas.eventEnergy;
  // default to the raw energy unless calibrated
  hit.energy = (double)meas.eventEnergy;
  hit.time = meas.eventTime;
  hit.indx = indx;
  hit.outOfRange = meas.outOfRange;
  for (int i = 0; i < 8; ++i) {
    hit.QDCSums[i] = meas.QDCSums[i];
  }
  // if(meas.traceLength > 0) {
  //   hit.trace = &meas.trace[0];
  //   hit.tracelen = meas.traceLength;
  // }
  hits.push_back(hit);
  return;
}
...
void DSSD::DSSDHitAnalysis(DSSDBeta *beta, DSSDImplant *implant) {
  ...
  double max_x_en[MAX_DSSDS] = { -1. };
  double max_y_en[MAX_DSSDS] = { -1. };
  ...
  for (auto &hit : hits) {
    ...
    // X position
    if (gain == 0) {
      // std::cout<<"x "<<strip<<" "<<gain<<" "<<dssdnum<<" "<<hit.energy<<"
      // "<<max_x_en[dssdnum]<<std::endl;
      ...
      // std::cout<<"y "<<strip<<" "<<gain<<" "<<dssdnum<<" "<<hit.energy<<"
      // "<<max_y_en[dssdnum]<<std::endl;
      if ((hit.energy > max_y_en[dssdnum]) && hit.energy < 3000) {
        if (!(dssdnum == 1 && (strip == 24 || strip == 0 || strip == 63))) {
          max_y_en[dssdnum] = hit.energy;
          ypos[dssdnum] = strip;
        }
        // std::cout << "y " << strip << " " << gain << " " << dssdnum << " " << hit.energy << "" << std::endl;
      }
    }
    ...
    if (gain == 2) {
      ...
      if (lg_first_e < hit.energy && dssdnum == 0) {
        lg_first_e = hit.energy;
        lg_first_y = strip;
      }
      if (lg_sec_e < hit.energy && dssdnum == 1) {
        lg_sec_e = hit.energy;
        lg_sec_y = strip;
      }

      // if (lg_last_e < hit.energy && dssdnum == 1) {
  ...
  if (e_max[0] > beta->upperthresh && lg_last_e > implant->lowerthresh) {
    implant->tke = lg_tke;
    implant->energy = lg_last_e;
    implant->energy0 = lg_first_e;
    implant->energy1 = lg_sec_e;
    ...
    implant->ypos0 = lg_first_y; // technically this is from MG, but should be the same
    implant->xpos1 = xpos[1];
    implant->ypos1 = lg_sec_y;
    // std::cout << "Implant valid: " << implant->valid << std::endl;
    // std::cout <<"Implant tke: " << implant->tke << std::endl;
    // std::cout << "Implant x: " << implant->xpos << " y: " << implant->ypos << " z: " << implant->zpos << std::endl;
    // } else if (e_max[0] <= beta->upperthresh && e_max[0] > 0 && e_max_zloc[0] >= 0 && ypos[e_max_zloc[0]] >= 0) {
  } else if (e_max[0] <= beta->upperthresh && e_max[0] > 0 && e_max_zloc[0] >= 0) {
    // decay when below beta threshold in HG
    // Use HG energy for best resolution
    beta->energy = e_max[0];
    beta->bkenergy = max_y_en[e_max_zloc[0]];
    ...
    // beta->valid = (beta->xpos != -1);
    //  std::cout<<"c "<<xpos[e_max_zloc[0]]<<" "<<ypos[e_max_zloc[0]]<<"
    //  "<<e_max_zloc[0]<<" "<<e_max_zloc[0]<<" "<<e_max_zloc[1]<<std::endl; std::cout<<"d
    //  "<<beta->xpos<<" "<<beta->ypos<<" "<<beta->zpos<<std::endl; std::cout <<
    //  "beta-valid: " << beta->valid << std::endl; std::cout << "beta energy: " <<
    //  beta->energy << std::endl;
  }
...
DSSDImplant::DSSDImplant() :
        TOF(0), dE(0), time(0), start(0), stop(0), xpos(-1), ypos(-1), zpos(-1),
        present(false), valid(false), nHits(0), lowerthresh(1), xtracelen(0),
        ytracelen(0) {
  // trace = NULL;
}
...
DSSDBeta::DSSDBeta() :
        energy(0), time(0), tdiff(-999), xpos(-1), ypos(-1), zpos(-1), present(false),
        valid(false), nHits(0), upperthresh(10000), xtracelen(0), ytracelen(0) {
  // trace = NULL;
}
...
Beta::Beta() :
        nCuts(0), time(0), present(false), valid(false), nHits(0), tdiff(-999),
        tracelen(0), upperthresh(60000) {
  for (int i = 0; i < 5; ++i) {
    energy[0][i] = 0;
    energy[1][i] = 0;
    energy[2][i] = 0;
...
SiPMBeta::SiPMBeta() :
        time(0), present(false), valid(false), nHits(0), tracelen(0), trace(NULL),
        upperthresh(60000) {
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 8; ++j) {
      energy[i][j] = 0;
...
Pin::Pin() : valid(false), ecal(0), gain(1), offset(0) {
  for (int i = 0; i < 2; ++i) {
    energy[i] = 0;
    time[i] = 0;
    fired[i] = 0;
...
Scint::Scint(int nch) : avtime(0), valid(false), nchans(nch) {
  for (int i = 0; i < nch; ++i) {
    energy[i] = 0;
    time[i] = 0;
    fired[i] = 0;
...
PPAC::PPAC() : valid(false), avtime(0), present(0) {
  for (int i = 0; i < 5; ++i) {
    fired[i] = 0;
    time[i] = 0;
    energy[i] = 0;
...
void pspmtCal::read_calgraphs(std::string file) {
  ...
  while (std::fgets(cline, sizeof cline, cfile) != NULL) {
    ...
    int ct = 0;
    while (ct < npoints) {
      if (std::fgets(cline, sizeof cline, cfile) == NULL) {
        return;
      }
...
std::pair<float, float> pspmtCal::interp2(float x, float y, float x1, float y1, float x2,
